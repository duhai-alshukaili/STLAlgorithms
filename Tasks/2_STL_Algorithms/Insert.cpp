...
*
* Task: Copy-and-paste the following code into 'quick-bench.com'. Benchmark the time to insert
*       elements in-order into a standard container.
*
**************************************************************************************************/
...
static void Vector(benchmark::State& state)
{
   for( auto _ : state )
   {
      ...
      for( const auto& item : initOrder ) {
         data.insert( std::lower_bound( begin(data), end(data), item ), item );
      }
