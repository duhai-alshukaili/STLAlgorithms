...
void find_youngest( Table const& table )
{
   ...
   // Classic algorithm solution
   auto const pos =
      std::min_element( std::begin(table), std::end(table), []( Person const& lhs, Person const& rhs ) {
...
void order_by_lastname( Table& table )
{
   ...
   // Classic algorithm solution
   std::stable_sort( std::begin(table), std::end(table), []( Person const& lhs, Person const& rhs ) {
...
void children_first( Table& table )
{
   ...
   // Classic algorithm solution
   std::partition( std::begin(table), std::end(table), []( Person const& p ) {
      return isChild( p );
   } );
...
void same_age( Table const& table )
{
   ...
   // Classic algorithm solution
   auto const pos =
      std::adjacent_find( std::begin(table), std::end(table), []( Person const& lhs, Person const& rhs ) {
         return lhs.age == rhs.age;
      } );
   ...
   /*
   auto const pos =
      std::ranges::adjacent_find( table, std::equal_to<>{}, &Person::age );
   */
...
void maximum_age_difference( Table const& table )
{
   ...
   */
   ...
   /*
   auto diff = 0;
   std::adjacent_find( std::begin(table), std::end(table), [&diff]( Person const& lhs, Person const& rhs ) {
   ...
   */
   ...
   /*
   auto diff = 0;
   std::ranges::adjacent_find( table, [&diff]( Person const& lhs, Person const& rhs ) {
      auto const ageDiff = std::abs( lhs.age - rhs.age );
      diff = std::max( diff, ageDiff );
      return false;
   } );
