...
void order_by_firstname( Table& table )
{
   ...
   // Classic algorithm solution
   std::sort( std::begin(table), std::end(table), []( Person const& lhs, Person const& rhs ) {
...
void simpsons_first( Table& table )
{
   ...
   // Classic algorithm solution
   std::stable_partition( std::begin(table), std::end(table), []( Person const& person ){
      return person.lastname == "Simpson";
   } );

   // C++20 ranges solution
   /*
   std::ranges::stable_partition( table, []( Person const& person ){
      return person.lastname == "Simpson";
   } );
   */
}
...
void compute_total_age( Table const& table )
{
   ...
   auto const age =
      std::accumulate( std::begin(table), std::end(table), int{}, []( int age, Person const& p ){
         return age + p.age;
      } );
